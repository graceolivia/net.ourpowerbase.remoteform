...
function remoteForm(config) {
  ...
  function createCheckboxesOrRadios(key, def, type) {
    ...
    var isPriceSet = /price_[0-9]+/.test(key);
    var isSingleOtherAmount = isPriceSet && 
                              Object.keys(def.options).length === 1 && 
                              isOtherAmountOption(def.options[Object.keys(def.options)[0]]);
  
    if (isPriceSet) {
      // Always create a hidden input for the price set
      var hiddenInput = document.createElement('input');
      hiddenInput.type = 'hidden';
      hiddenInput.name = key;
      hiddenInput.id = key;
      collectionDiv.appendChild(hiddenInput);
    }
  
    if (isSingleOtherAmount) {
      // Create the "Other Amount" text input for single other amount option
      var optionObj = def.options[Object.keys(def.options)[0]];
      var otherAmountDef = {
        'api.required': 1,
      ...
      optionsDiv.appendChild(otherAmountEl);
  
      // Add an event listener to update the hidden input when the amount changes
      var otherAmountInput = otherAmountEl.querySelector('input');
      otherAmountInput.addEventListener('input', function() {
        hiddenInput.value = this.value;
      });
    } else {
      // Handle multiple options (including price sets with multiple options)
      for (var optionId in def.options) {
        if (def.options.hasOwnProperty(optionId)) {
          ...
          if (isPriceSet) {
            var prefix = optionObj['currency'] == 'USD' ? '$' : '';
            ...
            if (isOtherAmountOption(optionObj)) {
              ...
              optionInput.addEventListener('click', function(e) {
                var referenceNode = document.getElementById(this.id).parentNode;
                var otherAmountDef = {
                  'api.required': 1,
                  title: 'Other Amount'
                };
                ...
                // Update hidden input when "Other Amount" changes
                var otherAmountInput = otherAmountEl.querySelector('input');
                otherAmountInput.addEventListener('input', function() {
                  hiddenInput.value = this.value;
                });
              ...
              optionInput.addEventListener('click', function(e) {
                hiddenInput.value = this.getAttribute('data-amount');
